...
function renderPipeDropdownMenu() {
  ...
  return m(".dropdown-menu", [
    ...
    m("button.dropdown-item", {
      onclick: function () {
        PD.state.pipeDropdownOpen = false;
        PD.actions.openEditor();
      },
    }, "Open in Editor"),
    m("a.dropdown-item", {
      href: "/traces",
      style: "text-decoration: none; color: var(--text-1);",
    }, "See Traces"),
  ]);
}
...
function renderPipeTraceItem(trace, traceIndex) {
  ...
  if (trace.durationMs != null) {
    summary += " (" + trace.durationMs.toFixed(1) + "ms, " +
      (trace.stepsTotal || "?") + " steps)";
  }
...
PD.components.PipeToolbar = {
  view: function () {
    ...
    return m("div", {
      style:
        "position: sticky; top: 0; z-index: 15; background: var(--surface-2);",
    }, [
      // ── Action buttons ──
      m(".toolbar-overlay", {
        style:
          "position: relative; opacity: 1; pointer-events: all; right: auto; transform: none; margin-block-end: var(--size-3);",
      }, [
        m("button.tb-btn", {
          onclick: function () {
            PD.actions.llmAction("description");
          },
        }, "Description"),
        m("button.tb-btn", {
          onclick: function () {
            PD.actions.llmAction("schema");
          },
        }, "Schema"),
        m("button.tb-btn", {
          onclick: function () {
            PD.actions.llmAction("tests");
          },
        }, "Tests"),
        // ── Split Run button ──
