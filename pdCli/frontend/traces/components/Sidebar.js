...
PD.components.Sidebar = {
  oncreate: function () {
    PD.actions.loadTraces();
  },
  view: function () {
    if (PD.state.loading) {
      return m(
        "div.sidebar",
        m("p", { style: "color: var(--text-2)" }, "Loading traces..."),
      );
    }
    ...
    Object.keys(grouped).sort().forEach(function (project) {
        ...
        m("button.project-group-header", {
          ...
          m(
            "span.project-group-caret" +
              (isProjectOpen ? ".project-group-caret--expanded" : ""),
            "\u25B8",
          ),
          m("span", project),
      ...
      if (isProjectOpen) {
        Object.keys(grouped[project]).sort().forEach(function (pipe) {
          ...
          const isPipeOpen = PD.state.expanded[pipeKey] === false;
          ...
          projectChildren.push(
            m("button.pipe-group-header", {
              ...
              m(
                "span.project-group-caret" +
                  (isPipeOpen ? ".project-group-caret--expanded" : ""),
                "\u25B8",
              ),
              m("span", pipe),
          ...
          if (isPipeOpen) {
            ...
            projectChildren.push(
              m(
                "div.project-group-pipes",
                grouped[project][pipe].map(function (entry) {
                  return m(
                    "div.trace-item" +
                      (PD.utils.isSelected(entry) ? ".active" : ""),
                    {
                      onclick: function () {
                        PD.actions.selectTrace(entry);
                      },
                    },
