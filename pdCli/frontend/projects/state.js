...
PD.utils.matchesSearch = function (project) {
  ...
  if (
    project.pipes && project.pipes.some(function (p) {
      return p.name.toLowerCase().includes(q);
    })
  ) return true;
  if (project.recentPipe && project.recentPipe.toLowerCase().includes(q)) {
    return true;
  }
  return false;
};
...
PD.actions.loadProjects = function () {
  return m.request({ method: "GET", url: "/api/projects" }).then(
    function (data) {
    ...
    const interval = setInterval(function () {
      ...
      if (!PD.state.pipesLoading || attempts > 50) {
        ...
        if (!PD.state.pipesLoading) {
          ...
          if (PD.state.viewingPipe && PD.state.viewingPipe.name === pipeName) {
            return;
          }
          const pipe = PD.state.focusedPipes.find(function (p) {
...
PD.actions.createNewProject = function () {
  ...
  const displayName = PD.state.newProjectName.trim();
  ...
  fetch("/api/projects", {
    ...
    setTimeout(function () {
      const match = PD.state.projects.find(function (p) {
...
PD.actions.createNewPipe = function () {
  ...
  const projectName = PD.state.focusedProject.name;
  const url = "/api/projects/" +
