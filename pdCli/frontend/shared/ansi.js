...
(function (pd) {
  ...
  pd.ansiToHtml = function (raw) {
    ...
    let italic = false;
    let underline = false;
    let fg = null; // e.g. "red", "bright-cyan"
    let bg = null; // e.g. "bg-yellow"

    // Walk through the string, splitting on CSI sequences.
    let out = "";
    let last = 0; // index past the previous match
    let match;
    ...
    while ((match = CSI_RE.exec(raw)) !== null) {
      ...
      const text = raw.substring(last, match.index);
      if (text) {
      ...
      const params = match[1] ? match[1].split(";") : ["0"];
      for (let i = 0; i < params.length; i++) {
        const code = parseInt(params[i], 10) || 0;
