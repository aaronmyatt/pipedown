...
(function () {
  ...
  function apply(theme) {
    ...
    const links = document.querySelectorAll('link[href*="highlightjs"]');
  ...
  let savedPref = null;
  try {
    savedPref = localStorage.getItem(STORAGE_KEY);
  } catch (_e) { /* ignore */ }
  // Validate — fall back to "auto" if the stored value is unexpected
  if (MODES.indexOf(savedPref) === -1) savedPref = "auto";

  // ── Initial application ──
  let currentPref = savedPref;
  let currentTheme = resolve(currentPref);
  apply(currentTheme);
  ...
  globalThis.pd.theme = {
    ...
    toggle: function () {
      const idx = MODES.indexOf(currentPref);
