...
(function(pd) {
  ...
  function expandRow(id) { jtOpen[id] = true; persistJtOpen(); }
  function collapseRow(id) { jtOpen[id] = false; persistJtOpen(); }
  ...
  */
  ...
  function jtNode(key, val, path) {
    ...
    const entries = agnosticEntries(val).map(pair => jtNode(pair[0], pair[1], id));
    return m("div", [
  ...
  function jsonTree(data, rootPath) {
    ...
    const entries = agnosticEntries(data).map(pair => jtNode(pair[0], pair[1], rootPath || "root"));
    return m(".jt", entries);
  }
