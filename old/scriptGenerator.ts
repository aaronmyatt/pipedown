...
const PIPE_TEMPLATE = (pipe, funcSequence) =>
  `import {pipeProcessor} from './pipeProcessor.js';

${
    funcSequence.map((func) =>
      `import {func${func.id}} from './out/scripts/func-${func.id}.js'`
    ).join("\n")
  }
...
async function generatePipeScript(
...
const FUNC_TEMPLATE = (func) =>
  `${[...func.code.matchAll(/^import.*/g, "")].join("\n")}
export default async function func${func.id}(input={}) {
${func.code.replaceAll(/^import.*/g, "")}
}
export {func${func.id}}`;

export async function generateFuncScript(
