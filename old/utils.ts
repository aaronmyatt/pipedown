...
export const pipeFileName = (dirname: string, id: string) =>
  `${Deno.cwd()}/${dirname}/pipes-${id}.json`;
export const funcFileName = (dirname: string, id: string) =>
  `${Deno.cwd()}/${dirname}/functions-${id}.json`;
export const pipeScriptName = (pipe) =>
  pipeFileName("out/scripts", pipe.id) + ".js";
export const pipeServerScriptName = (pipe) =>
  pipeFileName("out/scripts/server", pipe.id) + ".js";
export const pipeClientScriptName = (pipe) =>
  pipeFileName("out/scripts/client", pipe.id) + ".js";
export const writePipeDataToFile = (pipeData: Record<string, unknown>) =>
  writeDataToFile(pipeFileName(pipeDirName, pipeData.id), pipeData);
export const writeFuncDataToFile = (funcData: Record<string, unknown>) =>
  writeDataToFile(funcFileName(funcDirName, funcData.id), funcData);
export const writeFuncInputsDataToFile = (
  func: Record<string, unknown>,
  inputs: Record<string, unknown>,
) => writeLineToFile(funcFileName(inputsDirName, func.id), inputs);
export const writeFuncOutputsDataToFile = (
  func: Record<string, unknown>,
  outputs: Record<string, unknown>,
) => writeLineToFile(funcFileName(outputsDirName, func.id), outputs);
export const writePipeInputsDataToFile = (
  pipe: Record<string, unknown>,
  inputs: Record<string, unknown>,
) => writeLineToFile(pipeFileName(inputsDirName, pipe.id), inputs);
export const writePipeOutputsDataToFile = (
  pipe: Record<string, unknown>,
  outputs: Record<string, unknown>,
) => writeLineToFile(pipeFileName(outputsDirName, pipe.id), outputs);
...
export async function onePipe(pipeid) {
  return await allPipes().then((pipes) =>
    pipes.find((p) => p.id === Number(pipeid))
  );
}
...
export async function oneFunc(funcid) {
  return await allFuncs().then((funcs) =>
    funcs.find((f) => f.id === Number(funcid))
  );
}
...
export async function readWholeJSONDir(dirname: string) {
  ...
  for await (const file of Deno.readDir(`${Deno.cwd()}/${dirname}`)) {
    const fileData = await Deno.readTextFile(
      `${Deno.cwd()}/${dirname}/${file.name}`,
    );
...
export async function getPipeFunctions(pipe) {
  return await allFuncs().then((funcs) => {
    return funcs
      ...
      .sort((a, b) =>
        pipe.functions.indexOf(a.id) - pipe.functions.indexOf(b.id)
      );
  });
}
...
export const maybeworker = typeof WorkerGlobalScope !== "undefined" &&
