    ...
    get(target, prop, _receiver) {
        if (prop in target) {
            ...
            return async (pipeopts = {}) => {
                const inputs = Object.fromEntries(Object.entries(pipeopts).filter(([key, _value]) => !Object.keys(DEFAULT_OPTS).includes(key)));
                if(prop === 'temp'){
                    return executeScript({name: utils.pipeScriptName({id: 'temp'})}, inputs)
                }
